 ...
 */
@WebServlet("/LoginEngine")
public class LoginEngine extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) // form actionで送信ボタンを押した際に呼び出し
		...
		LoginLogic logic = new LoginLogic(); // LoginLogicクラスのインスタンスを生成
		...
		if (result == 0) {
			...
			// ここでDBからプロフィール情報を取得
			ProfileLogic profileLogic = new ProfileLogic(); // ProfileLogicクラスのインスタンスを生成
